...
// initialize USS related variables
  ...
  last_dist_raw = 0.0; // store the last distance and use if the next one is invalid
  light = 0.0;
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  last_dist_raw = reading; // store reading for next reading
  if(reading < dist_min || reading > dist_max) reading = last_dist_raw; // return 0 when out of range.
  return reading;
